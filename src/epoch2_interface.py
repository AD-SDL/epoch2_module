...
class Gen5Interface:
    ...
    def __init__(self, com_port=4, reader_type = 22, baud_rate = 38400):
        ...
        print("Communications passed" if comms_test == 1 else f"Communications test returned error code: {comms_test}")
    ...
    def run_experiment(self, experiment_file_path: WindowsPath, return_file: bool = False) -> Optional[WindowsPath]:
        """
        ...
        try:
            ...
            self.experiment = Gen5.Experiment(self.client.OpenExperiment(experiment_path_str))
            if self.experiment is None:
                raise FileNotFoundError(f"Failed to open experiment {experiment_path_str}")
            self.plates = Gen5.Plates(self.experiment.Plates)
            if self.plates is None:
                raise ValueError(f"Failed to get plates from experiment {experiment_path_str}")
            ...
            if self.plate_read_monitor.ErrorsCount > 0:
                error_message = "; ".join([f"[{self.plate_read_monitor.GetErrorCode(i)}] {self.plate_read_monitor.GetErrorMessage(ErrorIndex=i)}" for i in range(self.plate_read_monitor.ErrorsCount)])
                raise RuntimeError(f"{self.plate_read_monitor.ErrorsCount} error(s) reading plate: {error_message}")
            ...
            print("⚠️⚠️⚠️If prompted by the Jupyter Kernel to Restart, DON'T! (click 'cancel' to dismiss the dialog)⚠️⚠️⚠️")
        finally:
